  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-white">
      ...
      <div className="p-6 text-center border-t border-slate-50 bg-white">
          {isAdmin ? <button onClick={onGoToAdmin} className="inline-flex items-center gap-2 px-6 py-2 bg-blue-50 text-blue-600 rounded-full text-xs font-bold hover:bg-blue-100 transition-colors"><LayoutDashboard size={14}/> Ir al Panel de Admin</button> : <button onClick={onOpenLogin} className="text-[10px] font-bold text-slate-300 hover:text-slate-500 transition-colors uppercase tracking-widest">Acceso Corporativo</button>}
      </div>
    </div>
  );
}
...
export function LeadModal({ lead, onClose }) {
    ...
    const copyLeadToClipboard = () => { if (!lead) return; const text = [`📋 FICHA`, `Nombre: ${lead.nombre}`, `Email: ${lead.email}`, `Tel: ${lead.telefono}`, `Resumen: ${lead.resumen_ai}`].join('\n'); const textArea = document.createElement("textarea"); textArea.value = text; document.body.appendChild(textArea); textArea.select(); try { document.execCommand('copy'); setCopyFeedback(true); setTimeout(() => setCopyFeedback(false), 2000); } catch (err) {} document.body.removeChild(textArea); };
    return (
        <div className="fixed inset-0 z-[100] bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-2xl rounded-[24px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
                <div className="px-6 py-4 border-b flex justify-between items-center bg-slate-50"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center"><User size={20}/></div><div><h3 className="font-bold text-lg">{lead.nombre}</h3><p className="text-xs text-slate-500">{formatFirestoreDate(lead.createdAt)}</p></div></div><button onClick={onClose}><X size={20}/></button></div>
                <div className="p-6 overflow-y-auto space-y-6">
                    <div className="grid grid-cols-2 gap-4"><div className="p-4 bg-slate-50 rounded-xl"><span className="text-[10px] font-bold text-slate-400 uppercase">Email</span><p className="text-sm font-medium">{lead.email || 'No registrado'}</p></div><div className="p-4 bg-slate-50 rounded-xl"><span className="text-[10px] font-bold text-slate-400 uppercase">Teléfono</span><p className="text-sm font-medium">{lead.telefono || 'No registrado'}</p></div><div className="p-4 bg-slate-50 rounded-xl col-span-2 relative"><div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 rounded-l-xl"></div><span className="text-[10px] font-bold text-slate-400 uppercase flex gap-1 items-center"><Sparkles size={10}/> Resumen AI</span><p className="text-sm text-slate-700 mt-1 leading-relaxed">{lead.resumen_ai}</p></div></div>
                    <div className="space-y-3"><h4 className="text-xs font-bold text-slate-400 uppercase border-b pb-2">Historial de Chat</h4>{lead.fullChat?.map((m,i) => (<div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}><div className={`px-4 py-2 rounded-xl text-xs max-w-[85%] ${m.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800'}`}>{m.content}</div></div>))}</div>
                </div>
                <div className="p-4 border-t flex gap-2"><button onClick={copyLeadToClipboard} className={`flex-1 py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 ${copyFeedback ? 'bg-green-500 text-white' : 'bg-black text-white'}`}>{copyFeedback ? <CheckCircle size={16}/> : <Copy size={16}/>} {copyFeedback ? "¡Copiado!" : "Copiar Ficha"}</button></div>
            </div>
        </div>
    );
}
        ...
        <div className="max-w-5xl mx-auto animate-in fade-in">
             ...
             {isEditing ? (
                 <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 max-w-2xl mx-auto">
                     <h4 className="font-bold mb-4">{currentAgent.id?'Editar':'Nuevo'}</h4>
                     <form onSubmit={handleSaveAgent} className="space-y-4">
                         <div className="grid grid-cols-2 gap-4"><input required placeholder="Nombre" value={currentAgent.name} onChange={e=>setCurrentAgent({...currentAgent,name:e.target.value})} className="p-2 border rounded-lg text-sm"/><input required placeholder="Teléfono" value={currentAgent.phone} onChange={e=>setCurrentAgent({...currentAgent,phone:e.target.value})} className="p-2 border rounded-lg text-sm"/></div>
                         <input type="email" required placeholder="Email (Notificaciones)" value={currentAgent.email} onChange={e=>setCurrentAgent({...currentAgent,email:e.target.value})} className="w-full p-2 border rounded-lg text-sm"/>
                         <input placeholder="Foto URL" value={currentAgent.photoUrl} onChange={e=>setCurrentAgent({...currentAgent,photoUrl:e.target.value})} className="w-full p-2 border rounded-lg text-sm"/>
                         <input placeholder="Licencias" value={currentAgent.licenses} onChange={e=>setCurrentAgent({...currentAgent,licenses:e.target.value})} className="w-full p-2 border rounded-lg text-sm"/>
                         <div className="flex gap-3"><button type="button" onClick={()=>setIsEditing(false)} className="flex-1 py-2 bg-slate-100 rounded-lg text-sm">Cancelar</button><button type="submit" disabled={saving} className="flex-1 py-2 bg-black text-white rounded-lg text-sm">{saving?'Guardando...':'Guardar'}</button></div>
                     </form>
                 </div>
             ) : (
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                     {agents.map(agent=>(<div key={agent.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm relative group"><button onClick={()=>setAgentToDelete(agent)} className="absolute top-4 right-4 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button><div className="flex items-center gap-4 mb-3"><img src={agent.photoUrl||"https://ui-avatars.com/api/?name="+agent.name} className="w-12 h-12 rounded-full object-cover bg-slate-100"/><div><h4 className="font-bold text-sm">{agent.name}</h4><p className="text-xs text-slate-500">{agent.email}</p></div></div><div className="flex gap-2 mt-4"><button onClick={()=>{setCurrentAgent(agent);setIsEditing(true);}} className="flex-1 py-1.5 bg-slate-50 text-xs font-medium rounded-lg">Editar</button><button onClick={()=>onViewReport(agent)} className="flex-1 py-1.5 bg-blue-50 text-blue-600 text-xs font-medium rounded-lg flex items-center justify-center gap-1"><BarChart3 size={12}/> Reporte</button></div></div>))}
                 </div>
             )}
        </div>
    );
...
export function AdminBrain({ aiConfig, onSaveConfig }) {
  ...
  const handleTimeChange = (day, idx, field, val) => { setC(p => { const n={...p}; n.schedule[day].shifts[idx][field]=val; return n; }); };

  const save = async () => { 
      setSaving(true); 
      try { await onSaveConfig(c); setSaveMsg("¡Cambios guardados!"); } catch(e) { setSaveMsg("Error."); } 
...
export function ClientChat({ aiConfig, onSaveLead, onOpenLogin }) {
  ...
  const { isAgentAvailable, message, isVacation, resumeDate } = getAgentStatus(aiConfig);

  useEffect(()=> { const i = setInterval(() => setActiveUsers(p => p + (Math.random() > 0.5 ? 1 : -1)), 5000); return () => clearInterval(i); }, []);
  useEffect(()=> scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: 'smooth' }), [msgs, loading, showOptions]);

  const handleCopyLink = () => {
       const url = window.location.href;
       const ta = document.createElement("textarea"); ta.value = url; document.body.appendChild(ta); ta.select();
       try { document.execCommand('copy'); setCopySuccess(true); setTimeout(() => setCopySuccess(false), 2000); } catch (e) {}
  ...
  const callGemini = async (history, extra = "") => {
    try {
        ...
        if (isVacation && resumeDate) availabilityInstruction = `Estamos en vacaciones hasta el ${resumeDate.toLocaleDateString()}.`;

        const prompt = `FECHA: ${new Date().toLocaleString()}. ${aiConfig.systemPrompt} ${availabilityInstruction} HORARIO: ${scheduleText} HISTORIAL: ${history.map(m=>m.role+': '+m.content).join('\n')} ${extra} OBJETIVO: 7 datos (Nombre, Edad, Salud, Estado, Tabaco, Presupuesto, EMAIL). SI TIENES LOS 7 DATOS: Responde JSON { "action": "show_options", "text": "..." }. FINAL: JSON { "action": "save_lead", ... }`;
  ...
  const handleSend = (e) => { e.preventDefault(); if(!input.trim() || loading || ended) return; const h = [...msgs, {role:'user', content:input}]; setMsgs(h); setInput(''); setLoading(true); callGemini(h); };
  const handleOption = (t) => { setShowOptions(false); const h = [...msgs, {role:'user', content: t==='ahora'?'Hablar ahora':'Programar'}]; setMsgs(h); setLoading(true); callGemini(h, `[SISTEMA: Usuario eligió ${t}. Pide teléfono y valida horario.]`); };
    ...
    <div className="max-w-[480px] mx-auto flex flex-col h-full bg-white rounded-[32px] shadow-2xl border border-gray-100 overflow-hidden relative font-sans">
        ...
        {showShareModal && (
            <div className="fixed inset-0 z-[150] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
                <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm border border-gray-100 relative text-center">
                    ...
                    <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-4"><LinkIcon size={24} /></div>
                    <h3 className="font-bold text-lg text-slate-800 mb-2">Compartir Chat</h3>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-center gap-2 mb-4"><span className="text-xs text-slate-500 truncate flex-1 font-mono">{window.location.href}</span></div>
                    <button onClick={handleCopyLink} className={`w-full py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${copySuccess ? 'bg-green-500 text-white' : 'bg-black text-white'}`}>{copySuccess ? <><CheckCircle size={18} /> ¡Copiado!</> : "Copiar Enlace"}</button>
                </div>
            </div>
        )}
        
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-6 bg-white no-scrollbar">
            {msgs.map((m,i)=>(<div key={i} className={`flex ${m.role==='user'?'justify-end':'justify-start'} animate-in fade-in`}>{m.role === 'assistant' && <LucyAvatar className="w-8 h-8 mr-2 mt-auto shrink-0" />}<div className={`w-fit max-w-[75%] px-5 py-3 rounded-2xl text-[16px] leading-relaxed shadow-sm text-left ${m.role==='user'?'bg-[#007AFF] text-white rounded-br-none':'bg-[#F2F2F7] text-[#1d1d1f] rounded-bl-none'}`}><RichText content={m.content}/></div></div>))}
            {loading && <div className="flex justify-start pl-10"><div className="bg-[#F2F2F7] px-4 py-3 rounded-2xl rounded-bl-none flex gap-1.5 items-center w-fit"><span className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce"></span><span className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce delay-75"></span><span className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce delay-150"></span></div></div>}
            {showOptions && <div className="flex flex-col gap-2 pt-2 px-8"><button onClick={()=>handleOption('ahora')} disabled={!isAgentAvailable} className={`w-full font-medium py-3.5 rounded-xl text-sm shadow-sm flex items-center justify-center gap-2 ${isAgentAvailable?'bg-[#007AFF] text-white':'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>{isAgentAvailable?<Zap size={16}/>:<Moon size={16}/>} Hablar Ahora</button><button onClick={()=>handleOption('prog')} className="w-full bg-[#F2F2F7] text-[#007AFF] font-medium py-3.5 rounded-xl hover:bg-[#E5E5EA] transition-all text-sm flex items-center justify-center gap-2"><Calendar size={16}/> Programar</button></div>}
        </div>
        
        <form onSubmit={send} className="p-4 bg-white/90 backdrop-blur-xl border-t border-gray-100 flex gap-3"><input value={input} onChange={e=>setInput(e.target.value)} disabled={ended} placeholder={ended?"Chat finalizado":"Escribe un mensaje..."} className="flex-1 bg-[#F2F2F7] border-0 rounded-full px-5 py-3 text-[16px] focus:ring-2 focus:ring-[#007AFF]/20 outline-none"/><button disabled={loading||ended} className="w-12 h-12 bg-[#007AFF] text-white rounded-full flex items-center justify-center shadow-md"><Send size={20} className="ml-0.5" /></button></form>
...
export function App() {
  ...
  // --- SINCRONIZACIÓN DE DATOS (RUTAS PÚBLICAS) ---
  useEffect(() => {
    // IMPORTANTE: Ruta pública para que el Chat pueda escribir y el Admin leer
  ...
  // FUNCIONES DE BASE DE DATOS (USANDO RUTA PÚBLICA)
  const saveLeadToDb = async (d) => { await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'leads'), { ...d, createdAt: serverTimestamp(), status: 'active' }); };
  const saveConfig = async (c) => { await setDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'settings', 'config'), c); setAiConfig(c); };
  const deleteLead = async (ids) => { const batch = writeBatch(db); (Array.isArray(ids)?ids:[ids]).forEach(id => batch.delete(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leads', id))); await batch.commit(); };
  const updateStatus = async (ids, st) => { const batch = writeBatch(db); (Array.isArray(ids)?ids:[ids]).forEach(id => batch.update(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leads', id), { status: st })); await batch.commit(); };
  
  const assignAgent = async (ids, agent) => {
      const batch = writeBatch(db);
      (Array.isArray(ids)?ids:[ids]).forEach(id => {
          batch.update(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leads', id), { assignedAgentId: agent.id, assignedAgentName: agent.name, assignedAt: serverTimestamp(), status: 'assigned' });
  ...
  const unassignAgent = async (ids) => {
      ...
      (Array.isArray(ids)?ids:[ids]).forEach(id => {
          ...
          batch.update(ref, { assignedAgentId: deleteField(), assignedAgentName: deleteField(), assignedAt: deleteField(), status: 'active' });
      });
      await batch.commit();
  };

  const handleViewAgentReport = (agent) => { setAgentToAudit(agent); setAdminTab('reports'); };
