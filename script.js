...
// IMPORTACIÓN DE ICONOS (Verificada: Link renombrado a LinkIcon)
import { 
  ...
  Mail, UserMinus, UserPlus, Link as LinkIcon 
...
// CLAVE API (Tu clave actualizada)
...
// VALIDACIÓN DE HORARIOS (A prueba de fallos)
const getAgentStatus = (config) => {
    try {
        ...
        // 2. Verificar Día y Hora
        const days = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
        const dayName = days[now.getDay()];
        const sch = config?.schedule?.[dayName];
        
        // Si no hay configuración para hoy, asumimos cerrado por seguridad
        ...
        if (isNaN(sH) || isNaN(eH)) return { isAgentAvailable: true, message: "Disponible" }; // Fallback seguro

        const startMins = sH * 60 + (sM || 0);
        const endMins = eH * 60 + (eM || 0);

        if (mins < startMins || mins >= endMins) return { isAgentAvailable: false, message: "Cerrado ahora" };
        
        return { isAgentAvailable: true, message: "Agentes Disponibles" };
    } catch (error) {
        console.error("Error en horario:", error);
        return { isAgentAvailable: true, message: "Disponible" }; // Fallback final: siempre disponible si hay error
    }
};

// --- CONEXIÓN IA INTELIGENTE (Auto-Negociación de Modelo) ---
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar bg-white">
                    {assignedAgent && (
                        <div className="bg-blue-50 p-4 rounded-xl flex items-center gap-3 border border-blue-100 justify-between">
                            ...
                            {/* BOTÓN DESVINCULAR (Corregido) */}
                            {onAssignClick && (
                                <button onClick={() => onAssignClick([lead.id], 'unassign')} className="p-2 bg-white text-red-500 rounded-lg hover:bg-red-50 border border-transparent hover:border-red-100 transition-all" title="Desvincular Agente">
...
function App() {
    ...
    useEffect(() => {
        ...
        if (!user || !db) return; // DB Safety Check
    ...
    useEffect(() => {
        if (OFFLINE_MODE || !user || !isAdmin || !db) return; // DB Safety Check
