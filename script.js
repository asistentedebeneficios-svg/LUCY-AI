...
function formatFirestoreDate(ts) {
    ...
    try { return ts.toDate ? ts.toDate().toLocaleDateString('en-US') : new Date(ts.seconds * 1000).toLocaleDateString('en-US'); } 
...
// NUEVO FORMATO DE HORARIOS (CON SLOTS)
...
// --- VALIDACIÓN DE HORARIOS MULTI-SLOT ---
const getAgentStatus = (config) => {
    try {
        ...
        // Si el día está deshabilitado o no existe config
        if (!dayConfig || !dayConfig.enabled) return { isAgentAvailable: false, message: "Cerrado hoy" };

        // 3. Revisar SLOTS (Intervalos)
        const currentMins = now.getHours() * 60 + now.getMinutes();
        
        // Compatibilidad hacia atrás (si alguien tiene el formato viejo start/end directo)
        let slots = dayConfig.slots || [];
        if (!dayConfig.slots && dayConfig.start && dayConfig.end) {
            slots = [{ start: dayConfig.start, end: dayConfig.end }];
        }

        // Revisar si estamos dentro de ALGUNO de los slots
        ...
        if (!isOpenNow) return { isAgentAvailable: false, message: "Cerrado ahora" };
        
        return { isAgentAvailable: true, message: "Agentes Disponibles" };
    } catch (error) {
        console.error("Error horario:", error);
        return { isAgentAvailable: true, message: "Disponible" }; // Fallback
    }
};

// Generador de texto de horario para el Prompt de Lucy
const getScheduleText = (schedule) => {
    if (!schedule) return "";
    const days = ['lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo'];
    let text = "HORARIOS DE ATENCIÓN:\n";
    days.forEach(d => {
        const conf = schedule[d];
        if (conf?.enabled) {
            const slotsText = (conf.slots || [])
                .map(s => `${s.start}-${s.end}`)
                .join(' y ');
            // Compatibilidad legacy
            const legacyText = (!conf.slots && conf.start) ? `${conf.start}-${conf.end}` : "";
            text += `- ${d.charAt(0).toUpperCase() + d.slice(1)}: ${slotsText || legacyText}\n`;
        } else {
            text += `- ${d.charAt(0).toUpperCase() + d.slice(1)}: Cerrado\n`;
        }
    });
    return text;
};
...
async function fetchGeminiWithRetry(payload) {
    ...
    throw new Error(`Error conexión AI: ${lastError}`);
}
...
function App() {
    ...
    const handleAssignAgent = async (agentId) => {
        ...
        targetIds.forEach(id => { const ref = doc(db, 'artifacts', APP_ID, 'public', 'data', 'leads', id); batch.update(ref, { assignedAgentId: agentId === 'unassign' ? null : agentId, assignedAt: agentId === 'unassign' ? null : serverTimestamp() }); });
        await batch.commit();
        if (assignedAgent && aiConfig.assignmentWebhookUrl) {
            const targetLeads = leads.filter(l => targetIds.includes(l.id));
            targetLeads.forEach(lead => { if (lead.email) { const payload = { leadName: lead.nombre, leadEmail: lead.email, leadPhone: lead.telefono, agentName: assignedAgent.nombre, agentEmail: assignedAgent.email, agentPhone: assignedAgent.telefono, agentPhoto: assignedAgent.foto, assignedAt: new Date().toISOString() }; fetch(aiConfig.assignmentWebhookUrl, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).catch(err => console.error("Error webhook asignación:", err)); } });
        }
        setAssignModalData({ isOpen: false, targetIds: [] });
    };

    const openAssignModal = (ids, actionType) => { if (actionType === 'unassign') { if (confirm("¿Desvincular agente de este lead?")) { const batch = writeBatch(db); ids.forEach(id => { const ref = doc(db, 'artifacts', APP_ID, 'public', 'data', 'leads', id); batch.update(ref, { assignedAgentId: null, assignedAt: null }); }); batch.commit(); } } else { setAssignModalData({ isOpen: true, targetIds: ids }); } };
    const saveAgent = async (agentData) => { if (!isAdmin) return; const agentsRef = doc(db, 'artifacts', APP_ID, 'public', 'data', 'settings', 'agents_list'); const docSnap = await getDoc(agentsRef); let currentList = docSnap.exists() ? (docSnap.data().list || []) : []; if (agentData.id) { currentList = currentList.map(a => a.id === agentData.id ? { ...agentData, updatedAt: Date.now() } : a); } else { const newAgent = { ...agentData, id: generateId(), createdAt: Date.now() }; currentList.push(newAgent); } await setDoc(agentsRef, { list: currentList }); };
    const deleteAgent = async (ids) => { const idArray = Array.isArray(ids) ? ids : [ids]; if (!isAdmin) return; const agentsRef = doc(db, 'artifacts', APP_ID, 'public', 'data', 'settings', 'agents_list'); const docSnap = await getDoc(agentsRef); if (!docSnap.exists()) return; let currentList = docSnap.data().list || []; currentList = currentList.filter(a => !idArray.includes(a.id)); await setDoc(agentsRef, { list: currentList }); };
        ...
        <div className="h-[100dvh] bg-[#F5F5F7] text-[#1d1d1f] font-sans antialiased flex flex-col overflow-hidden">
            ...
            <nav className="bg-white/80 backdrop-blur-xl border-b border-white/20 shrink-0 z-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
                    <div className="flex items-center gap-3"><div className="bg-rose-500 text-white p-1.5 rounded-lg shadow-sm"><ProtectionLogo size={20} /></div><span className="font-semibold text-base sm:text-lg tracking-tight text-black">Asistente de Beneficios</span></div>
...
function AgentsManager({ agents, leads, onOpenLead, onSaveAgent, onDeleteAgent, searchTerm }) {
    ...
    const getAgentLeads = (agentId) => {
        ...
        const now = new Date();
        if (dateFilter === 'thisMonth') { agentLeads = agentLeads.filter(l => { const d = l.assignedAt?.toDate ? l.assignedAt.toDate() : new Date(l.assignedAt?.seconds * 1000); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); }); } 
        else if (dateFilter === 'lastMonth') { const lastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1); const endLastMonth = new Date(now.getFullYear(), now.getMonth(), 0); agentLeads = agentLeads.filter(l => { const d = l.assignedAt?.toDate ? l.assignedAt.toDate() : new Date(l.assignedAt?.seconds * 1000); return d >= lastMonth && d <= endLastMonth; }); } 
        else if (dateFilter === 'custom' && customStart && customEnd) { const start = new Date(customStart); const end = new Date(customEnd); end.setHours(23, 59, 59); agentLeads = agentLeads.filter(l => { const d = l.assignedAt?.toDate ? l.assignedAt.toDate() : new Date(l.assignedAt?.seconds * 1000); return d >= start && d <= end; }); }
        return agentLeads;
    };
        ...
        return (
            <div className="animate-in fade-in space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 items-center md:items-start">
                    ...
                    <div className="text-center bg-gray-50 p-4 rounded-xl"><span className="block text-3xl font-bold text-black">{agentLeads.length}</span><span className="text-xs text-gray-500 uppercase font-bold">Leads Asignados</span></div>
                </div>
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-xl border border-gray-100">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {['all', 'thisMonth', 'lastMonth', 'custom'].map(f => (<button key={f} onClick={() => setDateFilter(f)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${dateFilter === f ? 'bg-black text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{f === 'all' ? 'Todo' : f === 'thisMonth' ? 'Este Mes' : f === 'lastMonth' ? 'Mes Pasado' : 'Personalizado'}</button>))}
                    </div>
    ...
    return (
        <div className="animate-in fade-in space-y-6">
            <div className="flex justify-between items-center"><h2 className="text-xl font-bold text-gray-800">Gestión de Agentes</h2><div className="flex gap-2">{selectedIds.length > 0 && (<button onClick={handleDeleteSelected} className="bg-red-50 text-red-600 px-4 py-2 rounded-xl text-sm font-medium hover:bg-red-100 transition-colors flex items-center gap-2"><Trash2 size={16} /> Eliminar ({selectedIds.length})</button>)}<button onClick={() => openForm(null)} className="bg-black text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-gray-800 transition-colors flex items-center gap-2 shadow-lg shadow-gray-200"><UserCog size={16} /> Nuevo Agente</button></div></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAgents.map(agent => (
                    <div key={agent.id} className={`bg-white rounded-2xl p-5 border transition-all hover:shadow-md cursor-pointer relative group ${selectedIds.includes(agent.id) ? 'border-blue-500 bg-blue-50/10' : 'border-gray-100'}`} onClick={() => setSelectedAgent(agent)}>
                        ...
                        <div className="flex items-center gap-4 mb-4"><img src={agent.foto || "https://ui-avatars.com/api/?name=" + agent.nombre} className="w-14 h-14 rounded-full object-cover bg-gray-200" /><div><h3 className="font-bold text-gray-900">{agent.nombre}</h3><p className="text-xs text-gray-500 font-medium">{agent.email}</p></div></div>
                        <div className="space-y-2 text-xs text-gray-600"><div className="flex items-center gap-2"><Phone size={12} /> {agent.telefono || 'N/A'}</div><div className="flex items-center gap-2"><MapPin size={12} /> <span className="truncate max-w-[150px]">{agent.estados || 'Sin licencias'}</span></div></div>
                        <div className="mt-4 pt-4 border-t border-gray-50 flex justify-between items-center"><span className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Ver Bitácora</span><button onClick={(e) => { e.stopPropagation(); openForm(agent); }} className="p-2 bg-gray-50 rounded-full text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors"><Pencil size={14} /></button></div>
                    </div>
                ))}
            </div>
            {isEditing && (
                                ...
                                <div key={day} className="border-b border-slate-200/50 pb-3 last:border-0 last:pb-0">
                                    <div className="flex items-center justify-between group mb-2">
                                        ...
                                        {c.schedule?.[day]?.enabled && (
                                            <button onClick={() => handleAddSlot(day)} className="p-1 bg-slate-200 hover:bg-slate-300 rounded-full text-slate-600 transition-colors" title="Agregar turno"><Plus size={12} /></button>
                                        )}
                                    </div>
                                        ...
                                        <div className="space-y-2 pl-8">
                                            {(c.schedule[day].slots || []).map((slot, idx) => (
                                                <div key={idx} className="flex gap-2 items-center animate-in fade-in">
                                                    ...
                                                    <input type="time" value={slot.end} onChange={(e) => handleSlotChange(day, idx, 'end', e.target.value)} className="bg-white border border-slate-200 p-1 rounded text-xs w-20 text-center font-medium" />
        ...
        <div className="animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[24px] shadow-sm overflow-hidden border border-gray-100/50">
                ...
                <table className="w-full text-left table-fixed">
                    <thead className="bg-[#FBFBFD] border-b border-gray-100"><tr><th className="px-4 py-4 w-12 text-center"><input type="checkbox" className="custom-checkbox" checked={filteredLeads.length > 0 && selectedIds.length === filteredLeads.length} onChange={handleSelectAll} /></th><th className="px-4 py-4 text-[11px] font-bold text-[#86868b] uppercase w-1/4">Nombre</th><th className="px-4 py-4 text-[11px] font-bold text-[#86868b] uppercase w-1/4">Agente</th><th className="px-4 py-4 text-[11px] font-bold text-[#86868b] uppercase w-1/3">Resumen</th><th className="px-4 py-4 text-[11px] font-bold text-[#86868b] uppercase text-center w-32">Acción</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                        {filteredLeads.map(l => {
