...
function LandingView({ onStartChat, onOpenLogin, isAdmin, onGoToAdmin }) {
  ...
  // Responsive: cuantos testimonios mostrar
  useEffect(() => {
      const handleResize = () => {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-white">
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-4xl mx-auto space-y-10 animate-in slide-up">
        ...
        <div className="w-full mt-8 overflow-hidden">
            <div className="flex gap-4 justify-center transition-all duration-500 ease-in-out">
                {getVisibleTestimonials().map((t, i) => (
                    <div key={`${index}-${i}`} className="bg-slate-50 p-5 rounded-2xl border border-slate-100 flex-1 min-w-[280px] max-w-xs shadow-sm animate-in fade-in text-left">
